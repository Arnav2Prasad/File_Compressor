...
unsigned int* read_encoding_table(int from){
	...
	unsigned int *ch_arr = (unsigned int *)malloc(sizeof(unsigned int) * 2048);
	...
	while (read(from, &ch_from, 1))
	{
		if (ch_from == '$'){
			...
			printf("%s : %d\n", tmp, binary_to_decimal(tmp, i));

			ch_arr[binary_to_decimal(tmp,i)] = ch_from;
		}
...
char* decimal_to_binary(unsigned char ch){
	int a = (int)ch;
	int temp = a;
...
int recover_data(int from,unsigned char* m){
	char * to_name = "hiodofowjfspocjwopdfj";
	int to;
	to = open_file_writing(to_name);
	unsigned char ch;

	ll_char_queue *cq = ll_char_queue_init();
	while (read(to, &ch, 1))
	{
		char *temp = decimal_to_binary(ch);
		int i = 0;
		while (temp[i] != '\0'){
			ll_char_queue_insert(cq,(unsigned char)temp[i]);
			i++;
		}
		//Now we have inserted the binary code in the queue
		// Now we have to iterate the queue and locate the pattern in the hash map
...
ll_char_queue * ll_char_queue_init(void){
	ll_char_queue *temp = (ll_char_queue *)malloc(sizeof(ll_char_queue));
	if (temp){
		temp->front = NULL;
		temp->rear = NULL;
	}
...
void ll_char_queue_insert(ll_char_queue *q, unsigned char uc){
	ll_char_queue_node *temp = (ll_char_queue_node *)malloc(sizeof(ll_char_queue_node));
	temp->next = NULL;
	temp->data = uc;
	if (q->front == NULL)
	{
...
long long int ll_char_queue_isempty(ll_char_queue *q){
	return q->front == NULL;
}

unsigned char ll_char_queue_delete(ll_char_queue *q){
	if (q->front ==NULL){
		return 'a';
	}
	ll_char_queue_node *temp = q->front;
	q->front = q->front->next;
	if (q->front==NULL){
		q->rear = NULL;
	}
	unsigned char c= temp->data;
	free(temp);
	return c;
}

void ll_char_queue_print(ll_char_queue * q)
{
	ll_char_queue_node *temp = q->front;
	while (temp != NULL){
		printf("%c ", temp->data);
...
int main(int argc, char *argv[]) {
	...
	// fd = open_file_reading(strcat(argv[1], "_compressed"));
	fd = open_file_reading(argv[1]);

	

	unsigned int *arr;
	arr = read_encoding_table(fd);
